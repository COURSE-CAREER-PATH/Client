...
export const Input = ({ Labelvalue, Number, width, value, onChange})=>{
    ...
    <input type={`${Number? 'number':'text'}`} id="floating_outlined" className={`block px-2.5 pb-2.5 pt-4 text-sm text-neutral-300 bg-transparent border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-neutral-300 focus:outline-none focus:ring-0 focus:border-purple-700 peer border w-full py-2 rounded-xl ${width? 'md:w-auto w-full': ''}`} placeholder=" " required value={value} onChange={onChange}/>
